...
import MetaDetailPage from "./MetaProject/MetaDetailPage";
...
const Works = () => {
  ...
  if (showMeta) {
    return (
      <div>
        <button onClick={() => setShowMeta(false)} className="bg-tertiary text-white px-4 py-2 rounded mb-4">Back to Projects</button>
        <MetaDetailPage />
      </div>
    );
  }
      ...
      <div className="works-container mt-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-items-center gap-5">
        {projects.map((project, index) => (
          <div key={`project-${index}`} className="project-card">
            {project.name === "Meta" ? (
              <Link to="/projects/meta" style={{ display: 'block' }}>
                <ProjectCard index={index} {...project} />
              </Link>
            ) : project.name === "Swiggy" ? (
              <Link to="/projects/swiggy" style={{ display: 'block' }}>
                <ProjectCard index={index} {...project} />
              </Link>
            ) : project.name === "ChatGPT" ? (
              <Link to="/projects/chatgpt" style={{ display: 'block' }}>
                <ProjectCard index={index} {...project} />
              </Link>
            ) : (
              <ProjectCard index={index} {...project} />
